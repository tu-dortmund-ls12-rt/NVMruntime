...
static void byte_reverse(LONG *buffer, long count) {
    long i;
    BYTE ct[4] __attribute__((aligned(8))), *cp;
...
void sha_update(SHA_INFO *sha_info, BYTE *buffer, long count) {
...
void sha_stream(SHA_INFO *sha_info, char *data_input) {
    int i;
    BYTE data[BLOCK_SIZE];

    char *read_ptr = data_input;
...
void sha_print(SHA_INFO *sha_info) {
    log_info("sha result: " << hex << sha_info->digest[0] << hex
                            << sha_info->digest[1] << hex << sha_info->digest[2]
                            << hex << sha_info->digest[3] << hex
                            << sha_info->digest[4]);
}
